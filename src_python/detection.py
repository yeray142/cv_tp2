...
from bbox import non_maximum_suppression, xywh2xyxy
...
class Detector:
    def __init__(self):
        model_url = "https://github.com/yeray142/cv_tp2/releases/download/parameter/flappybird.pth"
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.net = YOLOv3(1)
        self.net.load_state_dict(torch.hub.load_state_dict_from_url(model_url))
        ...
        if not self.cap.isOpened():
            raise Exception("Camera is not detected")
    ...
    def get_pos(self):
        ...
        if not ret:
            raise Exception("cannot read image. It maybe use in other program")
        image = image[..., ::-1]
