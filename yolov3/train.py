...
def train_one_epoch(net: YOLOModelInterface, criterion: YOLOv3Loss, optimizer: torch.optim.Optimizer,
    ...
    if image is not None and target is not None:
        ...
        images = image.cpu().mul(255).add_(0.5).clamp_(0, 255).to("cpu", torch.uint8)[0]
        if preds is not None:
            images = draw_bounding_boxes(TF.resize(images, [416, 416]), xywh2xyxy(preds.cpu())[..., 0:4]).float().div(255)
        try:
            logger.log_image(images)
        except:
            pass
